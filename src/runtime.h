...
public:
    ...
    inline void Print(std::ostream& os, Context&) override {
        os << "Class " << name_;
    }
...
public:
    explicit ClassInstance(const Class& cls) : cls_(cls) {}

    // Возвращает true, если объект имеет метод method, принимающий argument_count параметров
    [[nodiscard]] inline bool HasMethod(const std::string& method,
                                        size_t argument_count = 0u) const {
        const Method* ptr = cls_.GetMethod(method);
        return ptr && ptr->formal_params.size() == argument_count;
    }

    // Возвращает ссылку на Closure, содержащий поля объекта
    [[nodiscard]] inline Closure& Fields() {
        return fields_;
    }

    // Возвращает константную ссылку на Closure, содержащую поля объекта
    [[nodiscard]] inline const Closure& Fields() const {
        return fields_;
    }
...
inline bool NotEqual(const ObjectHolder& lhs,
                     const ObjectHolder& rhs,
                     Context& context) {
...
inline bool Greater(const ObjectHolder& lhs,
                    const ObjectHolder& rhs,
                    Context& context) {
...
// Возвращает значение lhs<=rhs, используя функции Equal и Less
inline bool LessOrEqual(const ObjectHolder& lhs,
                        const ObjectHolder& rhs,
                        Context& context) {
    return Less(lhs, rhs, context) || Equal(lhs, rhs, context);
}
...
inline bool GreaterOrEqual(const ObjectHolder& lhs,
                           const ObjectHolder& rhs,
                           Context& context) {
